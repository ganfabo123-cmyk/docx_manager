...
class StyleTemplate:
    ...
    image_pPr:            object = None   # 图片段落格式（图片行 + caption 行共用）
...
def _extract_style(st: StyleTemplate, tag: str, elems: list,
    ...
    if tag == "表格":
        # 约定：块内第一行 <w:p> 是 caption 示例行（在 <w:tbl> 之前）
        for e in elems:
            local = e.tag.split("}")[-1]
            if local == "p" and st.table_caption_pPr is None:
                ...
                rPr = e.find(f".//{{{W}}}rPr")
                if pPr is not None:
                    st.table_caption_pPr = copy.deepcopy(pPr)
                if rPr is not None:
                    st.table_caption_rPr = copy.deepcopy(rPr)
            elif local == "tbl" and st.table_proto is None:
                st.table_proto = copy.deepcopy(e)

    elif tag in ("一级标题", "二级标题", "三级标题"):
        # 提取标题的 pPr（对齐/间距/缩进）和 rPr（字体/字号/加粗/颜色）
        key = {v: k for k, v in TYPE_TO_TAG.items()}[tag]   # "heading1" etc.
        for e in elems:
            if e.tag.split("}")[-1] == "p":
                pPr = e.find(f"{{{W}}}pPr")
                rPr = e.find(f".//{{{W}}}rPr")
                st.heading_styles[key] = HeadingStyle(
        ...
        # 提取正文段落的 pPr（首行缩进/行距）和 rPr（字体/字号）
        for e in elems:
            if e.tag.split("}")[-1] == "p":
                pPr = e.find(f"{{{W}}}pPr")
                rPr = e.find(f".//{{{W}}}rPr")
                if pPr is not None:
                    st.body_pPr = copy.deepcopy(pPr)
                if rPr is not None:
                    st.body_rPr = copy.deepcopy(rPr)
                break
        ...
        if len(paras) >= 2:
            rPr = paras[1].find(f".//{{{W}}}rPr")
            if rPr is not None:
                st.formula_label_rPr = copy.deepcopy(rPr)
...
def _add_heading_with_exclude(doc: Document, text: str, style_key: str,
                               exclude: bool = False,
                               st: StyleTemplate | None = None) -> None:
    """
    添加标题段落。格式优先级：
...
def _render_section(doc: Document, item: dict, st: StyleTemplate) -> None:
    ...
    if title_pPr is not None:
        ...
        _add_heading_with_exclude(doc, title, "heading1",
                                  exclude=exclude, st=st)

    # 正文（章节自己的 body_pPr 优先，否则用全局 [[正文]] 样式）
    body_pPr = ss.get("body_pPr") or st.body_pPr
    body_rPr = st.body_rPr
...
def generate(data: dict, st: StyleTemplate, output_path: str,
    ...
    for item in data.get("content", []):
        ...
        if t in ("heading1", "heading2", "heading3"):
            exclude = item.get("toc_exclude", False)
            _add_heading_with_exclude(doc, item["value"], t,
                                      exclude=exclude, st=st)
...
def _add_heading(doc: Document, text: str, style_key: str,
                 st: StyleTemplate | None = None):
    """兼容旧接口（内部渲染参考文献标题等场景）"""
    _add_heading_with_exclude(doc, text, style_key, exclude=False, st=st)
    ...
    for ref in refs:
        ...
        if st.ref_pPr_proto is not None:
            for child in st.ref_pPr_proto:
                if pPr.find(child.tag) is None:
                    pPr.append(copy.deepcopy(child))
        # 模板没有 [[参考文献]] 块时不加任何硬编码格式，保持 Word 默认
        run_num = para.add_run(f"[{ref['id']}] ")
        if st.body_rPr is not None:
            run_num._r.insert(0, copy.deepcopy(st.body_rPr))
        run_num.bold = True  # 编号始终加粗（语义要求，不是装饰格式）
        r_text = para.add_run(ref["text"])
        if st.body_rPr is not None:
            r_text._r.insert(0, copy.deepcopy(st.body_rPr))
...
if __name__ == "__main__":
    process(template_path='data/full_template_v6.docx',data_path='data/full_user_data_v6.json',output_path='data/output.docx')
